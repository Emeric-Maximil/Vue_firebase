<template>
    <div class="container list row m-auto">
        ...
        <div class="col-md-6">
            <ul class="list-group">
                <li v-for="(unArticle,index) in articles" 
				:key="index"
				@click="setActiveArticle(unArticle,index)"
				class="list-group-item">{{ unArticle.title }}</li>
            </ul>
            <button @click="removeAllArticles" class="m-3 btn btn-sm btn-danger">Tout Suppr.</button>
        </div>
        <div class="col-md-6">
            <div v-if="currentArticle">
                <article-details :article="this.currentArticle" @refreshList="refreshList" ></article-details>
            </div>
            <div v-else>
...
import ArticleDetails from '../components/Article.vue';
